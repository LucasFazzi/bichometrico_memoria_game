...
func _ready():
	click_enabled = true
	card_name = "Empty"
	card_face = preload("res://sprites/versocartas.png")
	card_back = preload("res://sprites/versocartas.png")
...
func on_click():
	if click_enabled:
		...
		if (get_parent().get_node("CardOneName").text == "Carta 1"):
			get_parent().get_node("CardOneName").text = card_name
			get_parent().card_one_checked_if_pairing = name
		elif (get_parent().get_node("CardOneName").text != "Carta 1"):
			if (get_parent().get_node("CardTwoName").text == "Carta 2"):
